...
const checkout = {
  ...
  click_dropdow_li: function (elm, t) {
    ...
    let getPackagePrice = parseFloat(
      $(elm).find(".get-package-price").attr("value")
    );
    ...
    $(elm)
      ...
      .parent()
      .find(s + "-package-id")
      .attr("value", getPackageId);
    $(elm)
      .parent()
      .parent()
      .find(s + "-package-price")
      .attr("value", getPackagePrice);
  ...
  add_cart: function (type, bid) {
    if (checkout.IsLogin === "1") {
      ...
      if (isCheck == true) {
        ...
        if (type == 1) window.location.href = "/checkout/cart";
      } else {
        checkout.noti("info", "Bạn chưa chọn sản phẩm nào");
      }
    } else {
      window.location.href =
        "/customer/login?redirect=" + window.location.pathname;
    }
  },
    ...
    $.ajax({
      ...
      success: function (item) {
        if (cartItem.BookType === 1) {
          let book_cost_value = parseFloat(
            $(elm).closest(".cart-item").find(".book-cost-value").val()
          );
          let book_price_value = parseFloat(
            $(elm).closest(".cart-item").find(".book-price-value").val()
          );
          ...
          $(elm)
            .closest(".cart-item")
            .find(".total-price-show")
            .html(total_price_value.format(0, 3, ".", ","));
          $(elm)
            .closest(".cart-item")
            .find(".total-cost-value")
            .val(total_cost_value);
          $(elm)
            .closest(".cart-item")
            .find(".total-discount-value")
            .val(total_discount_value);
        }
        checkout.calculate_money(true);
      },
    });
  ...
  address_dropdown: function () {
    ...
    if (addressId !== null && addressId !== "") {
      ...
      var districtId = $("option:selected", ".js-address-id").attr(
        "data-districtId"
      );
      ...
      var feeship = parseFloat(
        $("option:selected", ".js-address-id").attr("data-feeship")
      );
      var percentShipping = parseFloat($("#percentShipping").val());
      var discountShipping = (feeship * percentShipping) / 100;
      var TotalPayment =
        totalCost - discountbook - discountorder + feeship - discountShipping;
      $("#feeship").val(feeship);
      if (feeship == 0) {
      ...
      if (discountShipping > 0) {
        $(".discountShipping").html(
          "-" + discountShipping.format(0, 3, ".", ",") + "₫"
        );
  ...
  district_dropdown: function () {
    $(".js-district-select2").dropdown({
      ...
      choice: function (data) {
        $(".js-district-select2 .dropdown-selected").html(
  ...
  ward_dropdown: function (isVoucher) {
    $(".js-ward-select2").dropdown({
      ...
      choice: function (data) {
        $(".js-address-id").val("");
        $(".js-ward-select2 .dropdown-selected").html(
          data.currentTarget.textContent
        );
        ...
        function (data) {
          ...
          if (data.Type === 0) {
            ...
            $(".codeboxinput-apply").html(
              "Mã giảm giá không đúng, vui lòng kiểm tra lại."
            );
            ...
            data.PromotionBooks.forEach((item, index) => {
              ...
              let book_price_value = parseFloat(
                $(key).find(".book-price-value").val()
              );
              let quantity = parseInt($(key).find(".in-num").val());
              var book_discount_voucher =
                (book_price_value * item.Percent) / 100;
              var total_discount_voucher = book_discount_voucher * quantity;
              $(key)
                .find(".total-discount-voucher")
                .val(total_discount_voucher);
              if (index === len - 1) {
            ...
            $(".js-cart-discount-order").html(
              "-" + data.DiscountOrder.format(0, 3, ".", ",")
            );
            var total_feeship = parseFloat($(".bill-total-feeship").val());
            var discount_feeship = parseFloat(
              $(".bill-discount-feeship").val()
            );
            var TotalPayment =
              data.TotalPayment + total_feeship - discount_feeship;
            ...
            $(".discountShipping").html(
              "-" + data.DiscountShipping.format(0, 3, ".", ",")
            );
            $(".js-cart-payment").html(
              data.TotalPayment.format(0, 3, ".", ",")
            );
            $(".codeboxinput-apply").html("Đã áp dụng mã giảm giá.");
            $(".codeboxinput-apply").removeClass("error");
            $(".codeboxinput-apply").addClass("success");
            ...
            data.ListPaperId.forEach((i, id) => {
              ...
              data.PromotionBonusBooks.forEach((item, index) => {
                if (item.QueryId === i) {
                  var giftHtml =
                    '<a href="[url]" onclick="tracking([id])"> <i class="fas fa-gift mr-2"></i>Tặng kèm: [name][type][unit]</a>';
                  var url =
                    "/" + item.NameAscii + "-" + item.GiveQueryId + ".html";
            ...
            $(".total-discount-voucher").val(0);
            $(".codeboxinput-apply").html(
              "Chưa đủ điều kiện áp dụng mã giảm giá."
            );
            $(".codeboxinput-apply").addClass("error");
            $(".codeboxinput-apply").removeClass("success");
  ...
  change_feeship: function (isVoucher) {
    ...
    $(":button").prop("disabled", true);
    $.post(
      "/checkout/transportfee",
      {
        addressId: addressId,
        cityId: cityId,
        districtId: districtId,
        wardId: wardId,
        total_price_paper: TotalPricePaper,
    ...
    $(".cart-item").each(function (index) {
      ...
      if (checkbox.is(":checked")) {
        ...
        var total_cost_value = parseFloat(
          $(this).find(".total-cost-value").val()
        );
        var total_discount_value = parseFloat(
          $(this).find(".total-discount-value").val()
        );
        var total_discount_voucher = parseFloat(
          $(this).find(".total-discount-voucher").val()
        );
      ...
      if (index === len - 1) {
        ...
        $(".js-cart-discount-book").html(
          "-" + total_discount_book.format(0, 3, ".", ",")
        );
        ...
        var total_payment =
          total_price - discount_order + total_feeship - discount_order;
        ...
        if (p === 0) {
          $.post(
            "/checkout/promotion_total_ordervalue",
            { total_price: total_price },
            function (data) {
              $(".bill-discount-order").val(data.DiscountOrder);
              $(".js-cart-discount-order").html(
  ...
  dropdown_selected: function (elm, bid, e, p, price) {
    ...
    $(elm)
      .closest(".cart-item")
      .find(".total-price-show")
      .html(price.format(0, 3, ".", ","));
    $(elm)
      .closest(".cart-item")
      .find(".total-price-show-1")
      .html(price.format(0, 3, ".", ","));
    var text = $(elm).html();
  ...
  typicalSilder: function (elm, number, auto, margin) {
    var $owl = $(elm).owlCarousel({
      ...
      navText: [
        '<img src="' +
          UrlCdn +
          '/Content/assets/images/typical-book/prev.svg">',
        '<img src="' +
          UrlCdn +
          '/Content/assets/images/typical-book/next.svg">',
      ],
    });
...
const owl = {
  ...
  latestNewsSlider: function () {
    $(".latest-news__body-box").owlCarousel({
      ...
      nav: true,
      navText: [
        '<img src="' + UrlCdn + '/Content/assets/images/latest-news/left.svg">',
        '<img src="' +
          UrlCdn +
          '/Content/assets/images/latest-news/right.svg">',
      ],
      autoWidth: false,
      margin: 20,
    });
...
const homeSilder = {
  ...
  homeSlider: function () {
    var $owl = $(".home-slider .owl-carousel").owlCarousel({
      ...
      nav: true,
      navText: [
        '<img src="' + UrlCdn + '/Content/assets/images/home-slider/back.svg">',
        '<img src="' + UrlCdn + '/Content/assets/images/home-slider/next.svg">',
      ],
      autoWidth: false,
      margin: 10,
    });
...
const partnerAuthor = {
  ...
  partnerAuthorSlider: function () {
    ...
    $(".partner-author .partner-author__slider .owl-nav .owl-prev").html(
      '<img src="assets/images/prevsl.svg">'
    );
    $(".partner-author .partner-author__slider .owl-nav .owl-next").html(
      '<img src="assets/images/nextsl.svg">'
    );
    $(window).on("resize load", function () {
      wightScreen = screen.width;
      if (wightScreen < 767) {
        $(".partner-author .partner-author__slider .owl-nav .owl-next").html(
          '<img src="assets/images/arrowrmb.svg">'
        );
      } else {
        $(".partner-author .partner-author__slider .owl-nav .owl-next").html(
          '<img src="assets/images/nextsl.svg">'
...
$(document).ready(function () {
  $(".owl-carousel").owlCarousel({
    ...
    lazyLoad: true, // Kích hoạt lazy load cho hình ảnh
    // autoplay: true,                  // Tự động chuyển đổi slide
    // autoplayTimeout: 3000,           // Thời gian chờ giữa các slide (4 giây)
    // autoplayHoverPause: true,        // Dừng autoplay khi hover vào slider
    responsive: {
      0: { items: 1 }, // 1 item trên màn hình nhỏ
      600: { items: 1 }, // 1 item trên màn hình trung bình
      1000: { items: 1 }, // 1 item trên màn hình lớn
    },
